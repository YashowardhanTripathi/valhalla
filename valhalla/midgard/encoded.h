...
std::string encode(const container_t& points, const int precision = 1e6) {
  ...
  for (const auto& p : points) {
    ...
    int lon = static_cast<int>(floor(static_cast<double>(p.x()) * precision));
    int lat = static_cast<int>(floor(static_cast<double>(p.y()) * precision));
...
template <class container_t> std::string encode7(const container_t& points) {
  ...
  for (const auto& p : points) {
    ...
    int lon = static_cast<int>(floor(static_cast<double>(p.x()) * 1e6));
    int lat = static_cast<int>(floor(static_cast<double>(p.y()) * 1e6));
    // encode each coordinate, lat first for some reason
    serialize(lat - last_lat);
    serialize(lon - last_lon);
    // remember the last one we encountered
