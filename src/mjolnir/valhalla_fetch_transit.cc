...
std::priority_queue<weighted_tile_t> which_tiles(const ptree& pt, const std::string& feed) {
  ...
  for (const auto& feature : feeds.get_child("features")) {
    ...
    if (feed.empty() || (onestop_feed && *onestop_feed == feed)) {
      ...
      min_y -= std::abs(min_y - PointLL(min_x, min_y).MidPoint({max_x, min_y}).y());
      max_y += std::abs(max_y - PointLL(min_x, max_y).MidPoint({max_x, max_y}).y());
  ...
  for (const auto& tile : tiles) {
    ...
    auto min_y = std::max(bbox.miny(), bbox.minpt().MidPoint({bbox.maxx(), bbox.miny()}).y());
    auto max_y = std::min(bbox.maxy(), PointLL(bbox.minx(), bbox.maxy()).MidPoint(bbox.maxpt()).y());
...
void fetch_tiles(const ptree& pt,
  ...
  while (true) {
    ...
    auto min_y =
        filter.miny() -
        std::abs(filter.miny() - filter.minpt().MidPoint({filter.maxx(), filter.miny()}).y());
    auto max_y =
        filter.maxy() +
        std::abs(filter.maxy() - filter.maxpt().MidPoint({filter.minx(), filter.maxy()}).y());
